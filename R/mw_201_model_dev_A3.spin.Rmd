...
```{r ma5.4.1, fig.width=8, fig.height=12,  R.options = list(width = 1000)}
if(controls$rerun_models) {
...
```{r ma5.4.5a, fig.width=8, fig.height=12,  R.options = list(width = 1000)}
if(controls$rerun_models) {
  shapes_all = shapes$ara_shp %>%
    dplyr::filter(ara_id %in% ww_all$ara_id) %>%
    left_join(corr_all_ara,by = join_by(ara_id)) %>%
...
```
```{r ma5.4.5b, fig.width=8, fig.height=4,  R.options = list(width = 1000)}
avg_time_trend_reg(ww_all,ma5.4.5)
```
```{r ma5.4.5c, fig.width=8, fig.height=6,  R.options = list(width = 1000)}
mw_130_map_relative_vl(ma5.4.5,corr_all_ara,shapes)
```
```{r ma5.4.5d, fig.width=8, fig.height=6,  R.options = list(width = 1000)}
