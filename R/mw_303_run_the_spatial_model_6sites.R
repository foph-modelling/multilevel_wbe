...
selction_ids = readRDS('outputs/select_catchments_6.rds')

lab_adjust = merge(lab_method_ara, inla_results$res$summary.random$lab_method %>% select(ID, mean), by.x = 'lab_method', by.y='ID')
lab_adjust = lab_adjust %>% mutate(lab_adjust = exp(mean))

ww_all = ww_all %>% merge(lab_adjust %>% select(ara_id, lab_adjust), by='ara_id')


ww_all[, vl_stand := vl_stand * 1/lab_adjust]
ww_all[, vl := vl * 1/lab_adjust]

ww_all[, vl_smooth := zoo::rollmean(vl_stand, 7, na.rm=T, fill = NA), by=c('ara_id')]

#ww_all = readRDS('outputs/Full_run_6/ww_all_lab_adjust.rds')

saveRDS(ww_all, paste0(save.point, '/ww_all_lab_adjust.rds'))
for (selection in c(1, 2, 3, 4, 7)) {
  ara_ids = selction_ids[[selection]]
  ww_tofit = copy(ww_all)
  ww_tofit = ww_tofit[!(ara_id %in% ara_ids),  vl_stand := NA]
