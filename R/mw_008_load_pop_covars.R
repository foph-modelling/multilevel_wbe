...
mw_008_load_pop_covars = function(scale='ARA') {
  ...
  non_CH_EU = c('B22B28', 'B22B29', 'B22B30')
  select_cols = c('B22BTOT', males, fmale, non_CH_EU, "E_KOORD", "N_KOORD")
  ...
  if(scale == 'ARA'){
    ...
    catch_cov_cols = c('ara_id', 'PLZ', 'B22BTOT', males, fmale, non_CH_EU)
    
    catch_pops_covs = catch_pop_data_dt[, ..catch_cov_cols]
    catch_pops_covs_summed = catch_pops_covs[, lapply(.SD, sum, na.rm=TRUE), by=c('ara_id'), .SDcols=c('B22BTOT', males, fmale, non_CH_EU) ] 
    ...
    catch_pops_covs_summed[, prop_under_20 := rowSums(.SD) / B22BTOT, .SDcols = under_20_cols  ]
    catch_pops_covs_summed[, prop_over_65 := rowSums(.SD) / B22BTOT, .SDcols = over_65_cols  ]
    
    catch_pops_covs_summed[, non_ch_eu := rowSums(.SD), .SDcols = non_CH_EU]
    catch_pops_covs_summed[, prop_non_ch_eu := non_ch_eu/B22BTOT]
    ...
    catch_pops_covs_summed[, pop_dens := drop_units(B22BTOT/area)]
    
    
    catchement_population_covariates = catch_pops_covs_summed[, c('ara_id', 'B22BTOT', 'prop_under_20', 'prop_over_65', 'prop_non_ch_eu', 'pop_dens')]
    ...
    plz_age_cols = c('PLZ', 'B22BTOT', males, fmale)
    
    plz_pop_data_age_simp = plz_pop_data_age[, ..plz_age_cols]
    
    plz_pop_data_age_simp_summed = plz_pop_data_age_simp[, lapply(.SD, sum, na.rm=TRUE), by=PLZ, .SDcols=c('B22BTOT', males,fmale) ] 
    ...
    plz_pop_data_age_simp[, prop_under_20 := rowSums(.SD) / B22BTOT, .SDcols = under_20_cols  ]
    plz_pop_data_age_simp[, prop_over_65 := rowSums(.SD) / B22BTOT, .SDcols = over_65_cols  ]
    
    
    plz_eth_cols = c('PLZ','B22BTOT', non_CH_EU)
    
    plz_pop_data_birth = plz_pop_data_age[, ..plz_eth_cols]
    
    plz_pop_data_birth_summed = plz_pop_data_birth[, lapply(.SD, sum, na.rm=TRUE), by=PLZ, .SDcols=c('B22BTOT', non_CH_EU)] 
    
    plz_pop_data_birth = unique(plz_pop_data_birth_summed)
    plz_pop_data_birth[, non_ch_eu := rowSums(.SD), .SDcols = non_CH_EU]
    plz_pop_data_birth[, prop_non_ch_eu := non_ch_eu/B22BTOT]
    plz_pop_data_birth$prop_non_ch_eu = nafill(plz_pop_data_birth$prop_non_ch_eu,fill=0)
    
    plz_population_data = plz_pop_data_age_simp[, c('PLZ', 'B22BTOT', 'prop_under_20', 'prop_over_65')]
    ...
    plz_pop_cov[, pop_dens:=drop_units(B22BTOT/area)]
