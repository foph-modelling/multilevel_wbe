...
kclustsim = cluster::pam(mtx_dist, 6, diss=TRUE)
...
high_deg = features %>% arrange(-degree) %>% head(6)

high_bet = features %>% arrange(-betweenness) %>% head(6)

high_cen = features %>% arrange(-e_centrality) %>% head(6)

high_pra = features %>% arrange(-prank) %>% head(6)
...
write.csv(features, 'outputs/ara_selections_mobility_6.csv')
...
nuts_frame[top_nuts==T, top_nuts := !(nuts_frame[top_nuts==T, ]$total_pop == min(nuts_frame[top_nuts==T, ]$total_pop))]
...
saveRDS(object = select_catchments, file = 'outputs/select_catchments_6.rds')
