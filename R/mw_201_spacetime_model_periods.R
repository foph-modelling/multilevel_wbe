...
peri = 1 
period_dates = controls$period_dates
start_date = period_dates[peri]
if (peri < length(period_dates)){
  end_date = period_dates[peri+1]
}else{
  end_date = max(ww_all$date)
}
ww_all = ww_all %>% filter(date > start_date & date < end_date)
